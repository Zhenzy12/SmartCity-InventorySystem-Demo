...
const recoverCategories = () => {
  const selectedCategories = deletedCategoriesArray.value.filter(category => category.is_selected);
  console.log('Selected Categories:', selectedCategories);

  if (selectedCategories.length > 0) {
    showConfirmationModal.value = true;
  } else {
    console.log('Please select at least one category to recover.');
  }
}
...
const filteredDeletedCategories = ref([]);
const deletedCategoriesArray = ref([]);

onMounted(() => {
  // Fetch data from the database store
  databaseStore.fetchData();
  // Filter the deleted users
  filteredDeletedCategories.value = databaseStore.categoryList.filter(category =>
    category.is_deleted === 1
  );
  // Map the filtered users to include is_selected property
  deletedCategoriesArray.value = filteredDeletedCategories.value.map((category) => ({
        ...category,
        is_selected: false,
    }));
...
const confirmRecoverCategories = async () => {
    try {
        ...
        if (selectedCategories.length === 0) {
            emitter.emit("show-toast", { message: "Please select category to recover.", type: "error" });
            return;
        }
...
const handleCheckboxChange = (categoryId) => {
  deletedCategoriesArray.value = deletedCategoriesArray.value.map(category => ({
    ...category,
    is_selected: category.id === categoryId ? !category.is_selected : category.is_selected
  }));

  // Check if all users are selected
  const allSelected = deletedCategoriesArray.value.every(copy => copy.is_selected);
...
const handleCheckboxChangeAll = () => {
  if (checkedAll.value) {
    ...
    deletedCategoriesArray.value = deletedCategoriesArray.value.map(copy => ({
      ...copy,
      is_selected: false
    }));
  } else {
...
const filteredDeletedCategoriesSearch = computed(() => {
    return databaseStore.categoryList.filter(category =>
        category.is_deleted === 1 &&
...
watch(filteredDeletedCategoriesSearch, (newCategories) => {
    deletedCategoriesArray.value = newCategories.map((category) => ({
        ...category,
        is_selected: false,
    }));
});
...
<template>
  <div v-if="modelValue"
    ...
    <form class="flex items-center">
      ...
      <!-- FOR THE EQUIPMENT -->
      <div v-if="deletedCategoriesArray.length > 0"
        ...
        <div class="border w-fit min-w-full max-w-[600px] lg:max-w-full overflow-y-auto lg:mx-10 my-1 pb-1 rounded-lg dark:border-gray-700 ">
          <table class="w-full">
            ...
            <tbody class="w-full">
              <template v-for="category in deletedCategoriesArray">
                <tr 
                  class="w-full text-center items-center justify-center border-t">
                  <td class="pl-3 pt-1 w-10">
                    <label class="flex items-center cursor-pointer select-none text-dark dark:text-white">
                      <div class="relative">
                        <input type="checkbox" class="sr-only" @change="handleCheckboxChange(category.id)" />
                        <AkCheckBox v-if="category.is_selected" class="w-7 h-7" />
                        <AkBox v-else class="w-7 h-7" />
                      </div>
                    </label>
                  </td>
                  <td class="text-center">{{ category.category_name }}</td>
                  <td class="text-center">{{ formatDate(category.updated_at) }}</td>
                </tr>
              </template>
            </tbody>
          </table>
      ...
      <div v-else
        ...
        <p class="text-gray-500 dark:text-gray-400">No available categories to recover.</p>
      </div>
      ...
      <div class="-mx-3 flex flex-wrap mt-4">
        ...
        <div class="w-1/2 px-3">
          <button @click="recoverCategories()"
