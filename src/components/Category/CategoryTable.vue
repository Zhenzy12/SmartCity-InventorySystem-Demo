...
import RecoverCategoryModal from './Modals/RecoverCategoryModal.vue';
...
const isOpenRecoverCategoryModal = ref(false);

const emit = defineEmits(['open-add-category-modal', 'open-recover-category-modal'])

const OpenRecoverCategoryModal = () => {
    emit('open-recover-category-modal');
};
...
const filteredCategories = computed(() => {
    return databaseStore.categoryList
        ...
        .filter(category =>
            ...
            category.id.toString().includes(searchQuery.value.toLowerCase())
    ...
    <div>
        ...
        <div v-else class="overflow-x-auto">
            <div class="grid grid-cols-3 lg:grid-cols-8 gap-2 items-center justify-between md:space-y-0 md:space-x-4 p-4">
                ...
                <button @click.stop="OpenRecoverCategoryModal()"
                    class="col-span-1 w-full h-full flex items-center justify-center border px-2 py-1 rounded-lg button-default-green">
                    <MdRestore class="w-8 h-8" />
                    <p class="ml-1">Recover Categories</p>
                </button>
            </div>
                ...
                <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                    <thead class=" sticky top-0 z-10">
                        <tr
                            ...
                            <th class="py-3 px-2 cursor-pointer" @click="sortByField('category_name')">
                                <TableHeaderFormat label="Category Name" field="category_name" :sortBy="sortBy"
                                    :sortDirection="sortDirection" />
                            </th>
            ...
            <nav v-if="paginatedCategories.length > 0"
            ...
            <ViewItemsModal v-if="isOpenViewItemsModal" v-model="isOpenViewItemsModal"
                ...
                <RecoverCategoryModal v-if="isOpenRecoverCategoryModal" v-model="isOpenRecoverCategoryModal" @click. stop />
        </div>
    </div>
</template>