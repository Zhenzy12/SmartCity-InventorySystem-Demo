...
const fetchTotalUnreturnedItems = () => {
  ...
  databaseStore.transactionHistory.forEach(transaction => {
    if (transaction.return_date !== null) return; // only unreturned

    const borrower = databaseStore.borrowers.find(b => b.id === transaction.borrower_id);
    const borrowerName = borrower?.borrowers_name ?? 'Unknown';
    ...
    // Initialize or get existing borrower summary
    const current = borrowerMap.get(borrowerName) || {
      borrowerName,
    ...
    unreturnedItems.forEach(item => {
      // console.log("🚀 ~ fetchTotalUnreturnedItems ~ item:", item)
    ...
    borrowerMap.set(borrowerName, current);
  });

  borrowerUnreturnedItemsUnfiltered.value = Array.from(borrowerMap.values());
  
  if (currentFilter.value === "All Items") {
    borrowerUnreturnedItems.value = borrowerUnreturnedItemsUnfiltered.value;
  } else if (currentFilter.value === "Equipments") {
    borrowerUnreturnedItems.value = borrowerUnreturnedItemsUnfiltered.value.filter(borrower => borrower.totalUnreturnedEquipment > 0);
  } else if (currentFilter.value === "Supplies") {
    borrowerUnreturnedItems.value = borrowerUnreturnedItemsUnfiltered.value.filter(borrower => borrower.totalUnreturnedSupply > 0);
  }

  sortBorrowerData();
  // console.log("📊 Cleaned Borrower Data:", borrowerUnreturnedItems.value);
};
  ...
  <div class="w-full h-full">
    <div class="flex flex-row justify-between items-center mb-4">
      <label class="text-start text-lg 2xl:text-xl tracking-tight font-medium text-black dark:text-gray-200">
        Top Users with Unreturned Items:
      </label>
    ...
    <div
      ...
      <table class="w-full text-sm text-center text-slate-800 dark:text-slate-300">
        <thead>
          <tr class="uppercase text-center 2xl:text-lg ">
            <th class="py-4 text-center w-3/5">User</th>
      ...
      <div
        ...
        <table class="w-full text-md text-center ">
          <tbody>
            <tr v-for="borrower in filteredBorrowerUnreturnedItems" :key="borrower.id"
              ...
              <td v-if="itemTypeFilterItems[0].isActive && itemTypeFilterItems[1].isActive" class="py-3">
                {{ borrower.totalUnreturnedItems }}
              </td>
              <td v-if="itemTypeFilterItems[0].isActive && !itemTypeFilterItems[1].isActive" class="py-3">
                {{ borrower.totalUnreturnedEquipment || 0 }}
              </td>
              <td v-if="!itemTypeFilterItems[0].isActive && itemTypeFilterItems[1].isActive" class="py-3">
                {{ borrower.totalUnreturnedSupply }}
              </td>
            </tr>
          </tbody>
        </table>
