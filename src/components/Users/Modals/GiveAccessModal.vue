...
const confirmAddUser = async () => {
	try {
		...
		if (props.user.id) {
			const inventoryAccess = databaseStore.inventoryAccesses.find(
				(access) => access.user_id === props.user.id
			);
			if (inventoryAccess) {
				const accessData = {
					user_id: props.user.id,
					for_dashboard: data.value.access.for_dashboard ? true : false,
					for_transactions: data.value.access.for_transactions ? true : false,
					for_inventory: data.value.access.for_inventory ? true : false,
					for_categories: data.value.access.for_categories ? true : false,
					for_borrowers: data.value.access.for_borrowers ? true : false,
					for_offices: data.value.access.for_offices ? true : false,
					for_users: data.value.access.for_users ? true : false,
				};
		...
		emitter.emit("show-toast", {
			message: "Error adding access. Please try again.",
			type: "error",
		});
	} finally {
...
<template>
	<div
		...
		<div
			...
			<h3 class="text-3xl font-semibold mb-4">Give Access</h3>

			<!-- QUANTITY INPUT -->
			<div class="px-5 text-start max-h-[69vh] overflow-y-auto">
				<div class="px-5 mt-4">
					<label
						class="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-200"
					...
					<div class="grid grid-cols-2 gap-4">
						...
						<div class="flex items-center">
							<input
								...
								class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
							<label
								for="inventory_access"
								class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
								>Inventory Access</label
							>
						</div>
						...
						<div class="flex items-center">
							<input
								...
								class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
							<label
								for="categories_access"
								class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
								>Categories Access</label
							>
						</div>
						...
						<div class="flex items-center">
							<input
								...
								class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
							<label
								for="borrowers_access"
								class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"
								>Borrowers Access</label
							>
						</div>
			...
			<div class="-mx-3 flex flex-wrap mt-4">
				...
				<div class="w-1/2 px-3">
					<button
						@click="isClickedShowConfirmationModal()"
