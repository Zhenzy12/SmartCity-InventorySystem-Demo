...
watch(
	...
	() => {
		console.log(
			"🚀 ~ watch ~ props.transactionHistory:",
			props.transactionHistory
		);
		console.log(
			"🚀 ~ watch ~ props.selectedDateRange.start:",
			props.selectedDateRange.start
		);
		console.log(
			"🚀 ~ watch ~ props.selectedDateRange.end:",
...
const filteredTransactions = computed(() => {
  ...
  const startDate = props.selectedDateRange.start ? new Date(props.selectedDateRange.start) : null;
  const endDate = props.selectedDateRange.end ? new Date(props.selectedDateRange.end) : null;
  ...
  return props.transactionHistory.filter((transaction) => {
    ...
    // Office filtering
    if (
      activeOfficeIds.length > 0 &&
      !activeOfficeIds.includes(transaction.office_id)
    ) {
      return false;
    }
    ...
    // Borrower filtering
    if (
      activeBorrowerIds.length > 0 &&
      !activeBorrowerIds.includes(transaction.borrower_id)
    ) {
      return false;
    }
    ...
    // Lender filtering
    if (
      activeLenderIds.length > 0 &&
      !activeLenderIds.includes(transaction.lender_id)
    ) {
      return false;
    }
    ...
    if (activeReturnStatuses.length > 0) {
      ...
      const isReturned =
        activeReturnStatuses.includes("returned") && hasReturnDate;
      const isNotReturned =
        activeReturnStatuses.includes("not_returned") && !hasReturnDate;
    ...
    const borrower = props.borrowers?.find(
      (b) => b.id === transaction.borrower_id
    );
    ...
    const itemsMatch = selectedTransactionItems.some((item) => {
      if (item.item_type === "Office Supply") {
        ...
        return (
          equipmentName.includes(searchTerm) || equipmentId.includes(searchTerm)
        );
      }
      return false;
    });
		...
		const getFieldValue = (transaction, field) => {
			switch (field) {
				...
				case "borrower":
					return (
						props.borrowers
							.find((b) => b.id === transaction.borrower_id)
				...
				case "lender":
					return (
						props.users
							.find((u) => u.id === transaction.lender_id)
							?.firstName?.toLowerCase() || ""
					);
				...
				case "office":
					...
					return (
						props.officeList
							.find((o) => o.id === borrower?.office_id)
...
const toggleDropdown = (transactionId) => {
	openDropdownId.value =
		openDropdownId.value === transactionId ? null : transactionId;
};
...
const handlePrint = async () => {
    ...
    ${filteredTransactions.value
        ...
        <tr>
            <td>${report.id}</td>
            <td>${
							props.borrowers.find((b) => b.id === report.borrower_id)
								?.borrowers_name || "Unknown"
						}</td>
            <td>${
            ...
            <td>${
							(props.users.find((u) => u.id === report.lender_id)?.firstName ||
								"") +
							" " +
							(props.users.find((u) => u.id === report.lender_id)?.lastName ||
								"Unknown")
						}</td>
            ...
            <td>${props.transactionItems
							...
							.map((item) => {
								if (item.item_type === "Office Supply") {
									...
									const equipment = props.officeEquipments.find(
										(eq) =>
											eq.id ===
											props.equipmentCopies.find(
												(ec) => ec.id === item.item_copy_id
											)?.item_id
									);
									const copyNum = props.equipmentCopies.find(
										(ec) => ec.id === item.item_copy_id
									)?.copy_num;
									return `${
            ...
            <td>${
							report.return_date
								? new Date(report.return_date).toLocaleString()
								: "Not yet returned"
						}</td>
...
<template>
	<div class="w-full">
		<section class="w-full">
			<div class="shadow-lg rounded-lg">
				<div class="relative shadow-md sm:rounded-lg overflow-hidden">
					...
					<div
						class="grid grid-cols-2 lg:grid-cols-5 gap-2 items-center justify-between md:space-y-0 md:space-x-4 py-2">
						<!-- Search Box -->
						...
						<!-- ADD BUTTON -->
						<button
							@click.stop="OpenCreateTransactionModal()"
					...
					<!-- Second Row Filters -->
					<div
						class="grid grid-cols-6 lg:grid-cols-5 w-full justify-evenly gap-2 mb-2">
						<!--BORROWER DROPDOWN -->
						<div
							class="col-span-2 lg:col-span-1 relative w-full h-full text-left">
							<button
								@click="toggleborrowerDropDown"
								ref="borrowerDropDownButtonRef"
							...
							<div
								v-show="borrowerDropDownFilter"
								ref="borrowerDropDownMenuRef"
								...
								<div class="max-h-60 overflow-auto">
									<label
										...
										v-for="item in filteredBorrowerDropdown"
										:key="item.id">
										<div class="relative">
											<input
												type="checkbox"
												...
												@change="
													handleCheckboxChangeBorrower(item.id, $event)
												" />
											<div
						...
						<div
							...
							<div
								...
								<div class="max-h-60 overflow-auto">
									<label
										class="flex items-center cursor-pointer select-none text-dark dark:text-white mb-2"
										v-for="item in filteredOfficeDropdown"
										:key="item.id">
										<div class="relative">
											<input
												type="checkbox"
						...
						<div
							...
							<div
								...
								<div class="max-h-60 overflow-auto">
									<label
										class="flex items-center cursor-pointer select-none text-dark dark:text-white mb-2"
										v-for="item in filteredLenderDropdown"
										:key="item.id">
										<div class="relative">
											<input
												type="checkbox"
						...
						<!--ICS DROPDOWN -->
						<div
							class="col-start-2 col-span-2 lg:col-span-1 relative w-full h-full text-left">
							<button
								@click="toggleicsDropDown"
								ref="icsDropDownButtonRef"
								class="flex justify-center border w-full h-full items-center rounded-lg p-1 font-medium text-xs sm:text-sm md:text-base border-gray-400 bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-white dark:border-gray-600">
								<IcSolidFilter class="w-5 h-5 mr-1" />
								ICS Filter
								<MdOutlinedArrowDropDown class="w-5 h-5" />
							...
							<div
								v-show="icsDropDownFilter"
								ref="icsDropDownMenuRef"
								class="shadow-1 dark:shadow-box-dark absolute border border-gray-500 w-full right-0 z-40 mt-2 rounded-md bg-gray-200 dark:bg-gray-900 px-4 pt-2 transition-all">
								<div class="relative mb-2">
									<input
										type="text"
										v-model="icsSearchQuery"
										class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
								...
								<div class="max-h-60 overflow-auto">
									<label
										class="flex items-center cursor-pointer select-none text-dark dark:text-white mb-2"
										v-for="item in filteredIcsDropdown"
										:key="item.id">
										<div class="relative">
											<input
												type="checkbox"
						...
						<div
							class="col-span-2 lg:col-span-1 relative w-full h-full text-left">
							<button
								@click="toggleReturnStatusDropDown"
								ref="returnStatusDropDownButtonRef"
								class="flex justify-center border w-full h-full items-center rounded-lg p-1 font-medium text-xs sm:text-sm md:text-base border-gray-400 bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-white dark:border-gray-600">
								<IcSolidFilter class="w-5 h-5 mr-1" />
								Return Status
								<MdOutlinedArrowDropDown class="w-5 h-5" />
							...
							<div
								v-show="returnStatusDropDownFilter"
								ref="returnStatusDropDownMenuRef"
								class="shadow-1 dark:shadow-box-dark absolute border border-gray-500 w-full right-0 z-40 mt-2 rounded-md bg-gray-200 dark:bg-gray-900 px-4 pt-2 transition-all">
								<div class="relative mb-2">
									<input
										type="text"
										v-model="returnStatusSearchQuery"
										class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
								...
								<div class="max-h-60 overflow-auto">
									<label
										class="flex items-center cursor-pointer select-none text-dark dark:text-white mb-2"
										v-for="item in filteredReturnStatusDropdown"
										:key="item.id">
										<div class="relative">
											<input
												type="checkbox"
												...
												:checked="item.isActive"
												@change="
													handleCheckboxChangeReturnStatus(item.id, $event)
												" />
											<div
												class="box mr-4 flex h-5 w-5 items-center justify-center rounded border border-stroke dark:border-dark-3">
												<span
					...
					<!-- Table -->
					<div
						class="rounded-lg min-h-[49vh] max-h-[50vh] md:max-h-[53vh] xl:min-h-[57vh] xl:max-h-[58vh] 2xl:xl:max-h-[70vh] overflow-auto [scrollbar-width:thin] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden bg-gray-200 dark:bg-gray-900">
						<table
							...
							<thead class="sticky top-0 z-10">
								<tr
									...
									<th
										class="py-3 px-2 cursor-pointer"
									...
									<th
										class="py-3 px-2 min-w-40 cursor-pointer"
										@click="sortByField('borrower')">
										<TableHeaderFormat
											label="Borrower"
											field="borrower"
											:sortBy="sortBy"
									...
									<th
										class="py-3 px-2 min-w-50 cursor-pointer"
										@click="sortByField('office')">
										<TableHeaderFormat
											label="Office"
											field="office"
											:sortBy="sortBy"
									...
									<th
										class="py-3 px-2 min-w-30 cursor-pointer"
										@click="sortByField('return_date')">
										<TableHeaderFormat
											label="Return Date"
											field="return_date"
											:sortBy="sortBy"
							...
							<tbody class="">
								<template v-if="paginatedTransactions.length">
									<tr
										...
										<td class="px-4 py-3">
											{{
												props.borrowers.find(
													(borrower) =>
														Number(borrower.id) ===
														Number(transaction.borrower_id)
												)?.borrowers_name || "Unknown First Name"
											}}
										</td>
										<td class="px-4 py-3">
											{{
										...
										<td>
											<ul>
												<li
													v-for="item in props.transactionItems"
													:key="item.id"
												...
												<ul
													...
													<li
														...
														<UpdateModal
															...
															:officeEquipments="props.officeEquipments"
															:officeSupplies="props.officeSupplies"
															:equipmentCopies="props.equipmentCopies"
															:officeList="props.officeList"
		...
		<CreateTransactionModal
			v-if="isOpenCreateTransactionModal"
			v-model="isOpenCreateTransactionModal"
			@click.stop />
	</div>
</template>
